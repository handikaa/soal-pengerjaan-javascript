...
function tentukanKelulusan(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    arr[arr.length] = { name: data[i].name, score: data[i].score };
  }
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (result.zona.A.length < 4) {
      result.zona.A[result.zona.A.length] = p.name;
      result.pendaftar_lulus[result.pendaftar_lulus.length] = p;
    } else if (result.zona.B.length < 4) {
      result.zona.B[result.zona.B.length] = p.name;
      result.pendaftar_lulus[result.pendaftar_lulus.length] = p;
    } else if (result.zona.C.length < 4) {
      result.zona.C[result.zona.C.length] = p.name;
      result.pendaftar_lulus[result.pendaftar_lulus.length] = p;
    } else {
      result.pendaftar_tidak_lulus[result.pendaftar_tidak_lulus.length] = p;
    }
  }
